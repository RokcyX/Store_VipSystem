...
// gesture_password_open

- (UIViewController *)rootViewController {
    // 如果手势密码打开
    if ([Get_UserDefaults(Gesture_Password_Open) boolValue]) {
        // 手势密码界面
//        return [XYTabbarContorller merchantsTabbar];
        return [[XYBaseNavController alloc] initWithRootViewController:[[XYLoginViewController alloc] init]];
    }
    // 登录界面
    return nil;
}
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    Set_UserDefaults(@1, Gesture_Password_Open);
